...
第 5 章解决的是 Telegram 渠道接入，本章解决的是群组级路由：把不同群消息准确分发到对应 Agent，并设置兜底路由。

建议顺序：创建群并拿到群组 ID → 配置 `groups` 白名单 → 配置 `bindings` 精确匹配 → 最后加默认路由。
...
如果还要接入企业内协作渠道，继续 [第 7 章：接入飞书](./07-feishu.md)。
