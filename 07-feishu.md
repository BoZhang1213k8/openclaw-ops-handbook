...
本章专注“接入层”配置：应用创建、权限、长连接事件订阅、`channels.feishu` 访问控制。  
Agent 路由绑定放在下一章统一处理，避免“接入配置”和“路由配置”交叉导致理解断层。

建议顺序：创建并发布应用 → 配置 `channels.feishu` → 启用插件并建立长连接 → 验证群组消息进入网关。
...
> **Card 流式前置条件（重要）：**
> - `channels.feishu.renderMode` 需强制设为 `"card"`（见 7.2 配置示例）
> - 在飞书开发者平台权限管理中，需额外开通 **CardKit 的 3 个相关权限**（以平台页面展示名称为准）
> - 上述两项缺一时，飞书侧通常无法按卡片方式稳定展示流式输出
...
"channels": {
  "feishu": {
    ...
    "renderMode": "card",                        // 飞书强制使用 card 渲染（流式推荐）
    "accounts": {
      "main": {
        "appId": "<你的 App ID>",
...
并在飞书开发者平台中开通 CardKit 的 3 个相关权限（按平台实际权限名勾选）。实践上，`renderMode: "card"` + CardKit 权限齐全是飞书端流式卡片展示的关键条件。
...
继续 [第 8 章：为飞书绑定 Agent](./08-feishu-bindagent.md)，把飞书群组消息路由到具体 Agent（含 `accountId` 常见坑位）。
