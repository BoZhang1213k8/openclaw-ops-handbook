...
20. [接入钉钉应用机器人（对话通道）](./20-dingtalk-channel.md)
...
主线：`01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 20 → 10 → 11 → 12 → 14 → 13 → 17`  
按需分支：`09`（仅在需要“Skill 主动推送钉钉 Webhook”时阅读）

说明：
- 先完成 Agent 基础搭建，再按团队优先级接入任一对话通道（Telegram / 飞书 / 钉钉），随后进行 Skill 与安全加固，最后做自动化与性能优化。
- 第 20 章是钉钉对话通道接入（渠道插件模式）；第 9 章是 Skill 主动推送钉钉（Webhook 模式），按场景选读即可。
- Web（15）可在主链路稳定后再接入。
...
**钉钉对话通道章节完善与分享化改写** — 完成第 20 章的安装、权限、事件推送与分享口径补充，并同步修正 README 导航主线。

- **20 章补充**：新增 dingtalk 插件安装步骤（安装/启用/升级重装建议）
- **平台前置补充**：补齐“钉钉开发者平台创建应用、添加机器人、权限开通、发布生效”全流程
- **关键约束补充**：明确事件推送必须为 `Stream` 模式；手动开通权限基线为 `Card.Instance.Write` 与 `Card.Streaming.Write`
- **文档口径优化**：改为对外分享的可复用表达（示例值、适用前提、通用起步方案）
- **README 修正**：渠道分组标题、推荐阅读顺序与第 9/20 章定位说明对齐

### v2.9.0（2026-03-06）

**新增钉钉渠道级接入章节** — 基于 `openclaw.json`，补齐 `channels.dingtalk` 的配置总结与落地步骤，明确与第 9 章 Webhook 推送模式的边界。

- **新增第 20 章**：`channels.dingtalk` 渠道配置方法（字段说明、配置模板、路由建议、验证清单、常见排障）
- **README 更新**：在渠道目录新增第 20 章入口，区分“第 9 章通知推送”与“第 20 章对话通道”
